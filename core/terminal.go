...
func (t *Terminal) handleConfig(args []string) error {
	...
	if pn == 0 {
		...
		keys := []string{"domain", "external_ipv4", "bind_ipv4", "https_port", "dns_port", "unauth_url", "autocert", "webhook_url", "gophish admin_url", "gophish api_key", "gophish insecure"}
		vals := []string{t.cfg.general.Domain, t.cfg.general.ExternalIpv4, t.cfg.general.BindIpv4, strconv.Itoa(t.cfg.general.HttpsPort), strconv.Itoa(t.cfg.general.DnsPort), t.cfg.general.UnauthUrl, autocertOnOff, t.cfg.webhookUrl, t.cfg.GetGoPhishAdminUrl(), t.cfg.GetGoPhishApiKey(), gophishInsecure}
		log.Printf("\n%s\n", AsRows(keys, vals))
		return nil
	} else if pn == 2 {
		switch args[0] {
...
func (t *Terminal) createHelp() {
	...
	h.AddSubCommand("config", []string{"webhook_url"}, "kb_webhook_url <url>", "set the Keybase or Slack webhook URL")
